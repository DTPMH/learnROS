...
void Estimator::inputImage(double t, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    if(_img1.empty())
        ...
        featureFrame = featureTracker.trackImage(t, _img, _img1);//即两幅图像中的特征点的信息（位置，像素位置，相机id,特征点id,以及像素速度）放入featureFrame中
    ...
    if (SHOW_TRACK)//SHOW_TRACK==1
    {
        ...
        //std::cout<<"pub image time:"<<t<<std::endl;
    }
...
{
    mBuf.lock();
    featureBuf.push(make_pair(t, featureFrame));//把特征点信息放入featureBuf中
    mBuf.unlock();
        ...
        if(!featureBuf.empty())
        {
            ...
            if(USE_IMU)
            {
                if(!initFirstPoseFlag)//第一次处理时为0
                    initFirstIMUPose(accVector);//计算初始R0,并将....flag==1
