...
ros::Publisher cmd_vis_pub, pos_cmd_pub, traj_pub,my_pos_cmd_pub;

nav_msgs::Odometry odom;

quadrotor_msgs::PositionCommand cmd;
/*自己加的****************************/
geometry_msgs::PoseStamped my_pos_cmd;
/***********************************/
...
void drawCmd(const Eigen::Vector3d& pos, const Eigen::Vector3d& vec, const int& id,
             const Eigen::Vector4d& color,const Eigen::Vector3d& scale_) {
  ...
  mk_state.scale.x = scale_(0);;
  mk_state.scale.y = scale_(1);
  mk_state.scale.z = scale_(2);
...
void visCallback(const ros::TimerEvent& e) {
  ...
  displayTrajWithColor(traj_cmd_, 0.2, Eigen::Vector4d(0, 1, 0, 1), 2);
}
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  cmd.yaw = yaw;
  cmd.yaw_dot = yawdot;
  my_pos_cmd.pose.position.x=pos(0);
  my_pos_cmd.pose.position.y=pos(1);
  my_pos_cmd.pose.position.z=pos(2);
  ...
  my_pos_cmd_pub.publish(my_pos_cmd);

  // draw cmd

  drawCmd(pos, vel, 0, Eigen::Vector4d(0, 1, 0, 0.3),Eigen::Vector3d(0.1, 0.2, 0.3));
  drawCmd(pos, acc, 1, Eigen::Vector4d(0, 0, 1, 0.3),Eigen::Vector3d(0.1, 0.2, 0.3));

  Eigen::Vector3d dir(cos(yaw), sin(yaw), 0.0);
  drawCmd(pos, 2 * dir, 2, Eigen::Vector4d(1, 1, 0, 1),Eigen::Vector3d(0.3, 0.4, 0.5));
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber bspline_sub = node.subscribe("planning/bspline", 10, bsplineCallback);//有内容
  ros::Subscriber replan_sub = node.subscribe("planning/replan", 10, replanCallback);//无内容
  ros::Subscriber new_sub = node.subscribe("planning/new", 10, newCallback);//无内容
...
/**********************************下面是自己加的***************************************/
  my_pos_cmd_pub = node.advertise<geometry_msgs::PoseStamped>("/my_position_cmd", 10);
