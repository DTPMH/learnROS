...
uniform_real_distribution<double> rand_x;/*产生一个随机数*/
...
double theta_;
uniform_real_distribution<double> rand_radius_;/*产生随机数的意思*/
...
pcl::PointCloud<pcl::PointXYZ> cloudMap_rtab;
...
void RandomMapGenerate() {
  ...
  rand_x = uniform_real_distribution<double>(_x_l, _x_h);/*产生一系列-25到25的随机数*/
  rand_y = uniform_real_distribution<double>(_y_l, _y_h);/*产生一系列-25到25的随机数*/
  rand_w = uniform_real_distribution<double>(_w_l, _w_h);/*产生一系列0.3到0.8的随机数*/
  rand_h = uniform_real_distribution<double>(_h_l, _h_h);/*产生一系列3到7的随机数*/
  ...
  // generate polar obs
  for (int i = 0; i < _obs_num; i++) {/*obs_num=30表示障碍物的数量*/
    double x, y, w, h;
    x = rand_x(eng);
    y = rand_y(eng);
    w = rand_w(eng);

    if (sqrt(pow(x - _init_x, 2) + pow(y - _init_y, 2)) < 2.0) {/*_init_y,_init_x表示的建立的地图的原点。都为0*//*pow函数表示求X的Y次幂，sqrt表示开方*/
      i--;
      continue;
    }/*如果该随机产生的X，Y坐标与原点的距离小于2,则不做为障碍点*/
    ...
    for (int r = -widNum / 2.0; r < widNum / 2.0; r++)
      for (int s = -widNum / 2.0; s < widNum / 2.0; s++) {
        ...
        for (int t = -30; t < heiNum; t++) {
          pt_random.x = x + (r + 0.5) * _resolution + 1e-2;/*1e-2表示0.01*/
...
void usepcd()
{
 pcl::io::loadPCDFile<pcl::PointXYZ> ( "/home/dtpmh/work_space_all/dtpmh_ws/world_map.pcd", cloudMap_rtab );
std::cout<<"cloudMap_rtab大小:"<<cloudMap_rtab.size()<<std::endl;
 pcl::PointXYZ PTS;
 for (int i = 0; i < cloudMap_rtab.size(); ++i)
 {
  /*rtabmap需要的操作
 	PTS.x=-cloudMap_rtab[i].y;
 	PTS.y=cloudMap_rtab[i].x;
 	PTS.z=cloudMap_rtab[i].z;
  */
  PTS.x=cloudMap_rtab[i].x;
  PTS.y=cloudMap_rtab[i].y;
  PTS.z=cloudMap_rtab[i].z;
 	cloudMap.push_back(PTS);
 }
 std::cout<<"cloudMap大小:"<<cloudMap_rtab.size()<<std::endl;
}

void rcvOdometryCallbck(const nav_msgs::Odometry odom) {
  if (odom.child_frame_id == "X" || odom.child_frame_id == "O") return;
  _has_odom = true;/*将odom标志设为1*/
  ...
  _state = {odom.pose.pose.position.x,
            ...
            0.0,
            0.0,
            0.0};/*将odom的位置以及速度传给_state*/
}/*该函数的功能是将odom的位置以及速度传给_state*/

int i = 0;
void pubSensedPoints() {
  // if (i < 10) {
  pcl::toROSMsg(cloudMap, globalMap_pcd);/*将cloudmap转化为点与数据globalmap*/
  globalMap_pcd.header.frame_id = "world";
  _all_map_pub.publish(globalMap_pcd);/*将该信息发布*/
  // }
  ...
  pcl::PointXYZ searchPoint(_state[0], _state[1], _state[2]);/*将odom的坐标放入searchPoint中*/
  ...
  for (int r = -widNum / 2.0; r < widNum / 2.0; r++)
    for (int s = -widNum / 2.0; s < widNum / 2.0; s++) {
      ...
      for (int t = -1; t < heiNum; t++) {
        pt_random.x = x + (r + 2.5) * _resolution + 1e-2;
        pt_random.y = y + (s + 2.5) * _resolution + 1e-2;
        pt_random.z = (t + 0.5) * _resolution + 1e-2;
...
int main(int argc, char** argv) {
  ...
  _local_map_pub = n.advertise<sensor_msgs::PointCloud2>("/map_generator/local_cloud", 1);/*发布局部地图信息*/
  _all_map_pub = n.advertise<sensor_msgs::PointCloud2>("/map_generator/global_cloud", 1);/*发布全局地图信息*/

  _odom_sub = n.subscribe("odometry", 50, rcvOdometryCallbck);/*接收到odom的时候进行回调函数，将odom的信息传给_state*/

  click_map_pub_ =
      n.advertise<sensor_msgs::PointCloud2>("/pcl_render_node/local_map", 1);/*发布局部地图*/
  // ros::Subscriber click_sub = n.subscribe("/goal", 10, clickCallback);

  n.param("init_state_x", _init_x, 0.0);
  n.param("init_state_y", _init_y, 0.0);/*定义初始状态的X，Y值为（0,0）*/
  ...
  n.param("map/circle_num", circle_num_, 30);/*定义地图大小以及障碍物数量*/
  ...
  n.param("sensing/radius", _sense_rate, 10.0);/*以上定义的都是障碍物的参数*/
  ...
  _obs_num = min(_obs_num, (int)_x_size * 10);/*定义障碍物的数量*/
  _z_limit = _z_size;/*限定Z轴的最高值*/

  ros::Duration(0.5).sleep();/*休息0.5s*/

  RandomMapGenerate();/*产生随机地图*/
  //usepcd();
//pcl::io::loadPCDFile<pcl::PointXYZ> ( "/home/dtpmh/work_space_all/dtpmh_ws/world_map.pcd", cloudMap );
  ros::Rate loop_rate(_sense_rate);/*个ros::Rate对象允许你制定循环的频率*/

  while (ros::ok()) {
    pubSensedPoints();/*将以odom为中心的局部地图发布*/
    ros::spinOnce();/*返回后继续执行程序*/
    loop_rate.sleep();
  }
}
